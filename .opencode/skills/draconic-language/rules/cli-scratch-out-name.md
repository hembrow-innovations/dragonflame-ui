...
When `-o` is omitted, `draconic build` writes `{stem}.out.js` (js) or `{stem}.out` (native) beside the input. Gitignore covers `*.out.js`, `*.out.js.map`, `*.out`, and `*.dSYM`.
...
Omit `-o` for throwaway emit (gitignored). Pass `-o` only when the artifact should be kept.

**Notes.** `run` still builds in a temp dir. Explicit `-o hello.js` is a kept file, not scratch. See `cli-build-requires-target`.
