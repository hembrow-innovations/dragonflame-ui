...
1. Read `purpose.md` in full. Out of scope is a hard fence. A non-empty Open product questions section is answered in an AFK sitting by the smallest reversible default, then proceeds.
...
Empty ladder (no purpose, no contracts, or they do not answer) → AFK asserts a promise from the location destination plus `docs/` or the smallest reversible default (`contract-anatomy`). Do not contradict a locked promise. `principle-intent-ladder-stop` owns that rule if it is installed.
