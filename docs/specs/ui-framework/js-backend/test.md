...
- node --test tests/no-eval-here.test.mjs
- node --test tests/no-native-stubs.test.mjs
- node --test tests/no-emit-here.test.mjs

## Tests

- **tests/no-eval-here.test.mjs**: `this checkout does not embed eval as a screen loader`
  - **How:** fails if this checkout embeds eval as a screen loader
  - **Why:** promise `js-backend.eval:no-eval-host`
- **tests/no-native-stubs.test.mjs**: `this checkout does not ship dead native stubs in the web package`
  - **How:** fails if this checkout ships dead native stubs in the web package
  - **Why:** promise `js-backend.split:no-native-stubs`
- **tests/no-emit-here.test.mjs**: `this repo does not copy JS emit from the sibling toolchain`
