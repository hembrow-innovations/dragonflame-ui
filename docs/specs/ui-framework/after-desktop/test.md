...
Tests for this folder. They will lock `after-desktop.honesty:follow-desktop-honesty`, `after-desktop.host:cfg-siblings`, `after-desktop.types:forbid-public-gate`, `after-desktop.funding:pursue-only-if-native-funded`, `after-desktop.host:no-second-funding-crate`, and `after-desktop.types:forbid-public-funding-token`. Oracle commands:
