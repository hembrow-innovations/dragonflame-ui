...
- node --test tests/composite-h.test.mjs
- node --test tests/composite-children.test.mjs
- node --test tests/composite-run-once.test.mjs

## Tests

- **tests/composite-h.test.mjs**: `function type mounts to host leaves`
...
- **tests/composite-children.test.mjs**: `props.children nest through the composite onto DOM`
  - **How:** nested children in `props.children` pass through a composite and appear on DOM
  - **Why:** promise `composite.children:nest`
- **tests/composite-run-once.test.mjs**: `composite function did not re-run on a text patch`
