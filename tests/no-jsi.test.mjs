...
function isJsiPath(r) {
	return /jsi/i.test(r);
}

function isJsiSource(src) {
	return /\bjsi\b/i.test(src);
}

test("this checkout does not steal JSI as host config", () => {
	const files = walk(root).filter((path) => path !== self && !isNote(path));
	assert.deepEqual(
		files.filter((path) => isJsiPath(rel(path))),
		[],
	);
	assert.deepEqual(
		walk(join(root, "src")).filter((path) => isJsiSource(readFileSync(path, "utf8"))),
		[],
	);
